...
export async function GET() {
  ...
  // Agregar datos mock
  const data = [
    { worker_id: "1234567890", worker_name: "Andrés Felipe Gómez", company: "Metalprest", reason: "ARL Vencida (Bloqueo HSE)", hse_status: "NON_COMPLIANT" },
    { worker_id: "9876543210", worker_name: "Luis Fernando Ruiz", company: "Soldetech", reason: "Inasistencia No Registrada", hse_status: "COMPLIANT" }
  ];
  ...
  data.forEach((item) => {
    ...
    if (item.hse_status === 'NON_COMPLIANT') {
      statusCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFCA5A5' } }; // Rojo claro
      statusCell.font = { color: { argb: 'FF991B1B' }, bold: true };
    } else {
      statusCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF86EFAC' } }; // Verde claro
  ...
  return new NextResponse(buffer, {
    ...
    headers: {
      ...
      'Content-Disposition': 'attachment; filename="Reporte_Faltantes_HSE.xlsx"',
    },
  });
}
