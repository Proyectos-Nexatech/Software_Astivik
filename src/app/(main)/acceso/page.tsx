...
export default function ControlAccesoPage() {
  ...
  const fetchRegistrosRecientes = async () => {
    let query = supabase
      ...
      .select(
        "*, trabajadores(nombre, empresa, documento), proyectos(nombre), visitantes(nombre, empresa_origen, documento)",
      )
    ...
    if (filtroTiempoVivo === "hoy") {
      ...
      const localISOTime =
        new Date(today.getTime() - tzOffset).toISOString().split("T")[0] +
        "T00:00:00.000Z";
      query = query.gte("fecha_hora", localISOTime);
    } else if (filtroTiempoVivo === "mes") {
      const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
      const localISOTime =
        new Date(startOfMonth.getTime() - tzOffset)
          .toISOString()
      ...
      const localISOTime =
        new Date(startOfYear.getTime() - tzOffset).toISOString().split("T")[0] +
        "T00:00:00.000Z";
      query = query.gte("fecha_hora", localISOTime);
    }
  ...
  const handleBuscar = async (e: React.FormEvent) => {
    ...
    const { data: tData, error: tError } = await supabase
      .from("trabajadores")
      .select("*")
      .eq("documento", documentoBusqueda.trim())
      .single();
    ...
    if (tError || !tData) {
      ...
      if (vError || !vData) {
        setEstadoAcceso({
          ...
          detalles: [
            "La cédula no pertenece a un trabajador ni a un visitante autorizado.",
            "Por favor, diríjase a RRHH o solicite un pase de visitante.",
          ],
        });
        return;
      }
      ...
      setTrabajadorActual({
        id: vData.id,
        nombre: vData.nombre,
        empresa: vData.empresa_origen || "Visitante",
        cargo: "VISITANTE",
        es_visitante: true,
      });

      // Revisar si ya está adentro
      const { data: lastAccessV } = await supabase
        .from("registros_acceso")
        .select("*")
        .eq("visitante_id", vData.id)
        .order("fecha_hora", { ascending: false })
        .limit(1)
        .single();
      ...
      if (isInsideV) {
        setEstadoAcceso({
          tipo: "SALIDA_LIBRE",
          mensaje: "Visitante en Planta",
          detalles: ["Salida de visitante autorizada."],
        });
        return;
      }
      ...
      if (now < dInicio) {
        setEstadoAcceso({
          tipo: "BLOQUEADO",
          mensaje: "ACCESO AÚN NO VÁLIDO",
          detalles: [
            `El pase de visita inicia el: ${dInicio.toLocaleString()}`,
          ],
        });
      } else if (now > dFin) {
        setEstadoAcceso({
          tipo: "BLOQUEADO",
          mensaje: "PASE VENCIDO",
          detalles: [`El pase expiró el: ${dFin.toLocaleString()}`],
        });
      } else {
        setEstadoAcceso({
          tipo: "PERMITIDO",
          mensaje: "VISITANTE AUTORIZADO",
          detalles: ["Pase activo en el rango permitido."],
        });
      }
      return;
    }
    ...
    if (isInside) {
      ...
      setEstadoAcceso({
        ...
        detalles: [
          "Salida libre autorizada.",
          "No se requieren verificaciones HSE para la salida.",
        ],
      });
      return;
    }
    ...
    if (detallesVencidos.length > 0) {
      ...
      setEstadoAcceso({
        ...
        detalles: ["Todos los documentos HSE están al día y aprobados."],
      });
    }
  };
  ...
  const registrarAcceso = async (tipo: "ENTRADA" | "SALIDA") => {
    ...
    const payload: any = {
      tipo: tipo,
      proyecto_id:
        proyectoSeleccionado && proyectoSeleccionado !== "none"
  ...
  const exportarCsv = () => {
    ...
    const exportFormat = reportData.map((r) => ({
      ...
      Empresa:
        r.trabajadores?.empresa || r.visitantes?.empresa_origen || "Visitante",
      Tipo: r.tipo,
  ...
  const registrosFiltrados = registrosRecientes.filter((r) => {
    if (transitosFiltroTipo !== "TODOS" && r.tipo !== transitosFiltroTipo)
      return false;
    if (transitosSearch) {
  ...
  const registrosAgrupados = registrosPaginados.reduce(
    (acc, curr) => {
      const fecha = new Date(curr.fecha_hora);
      const dateKey = fecha.toLocaleDateString("es-ES", {
        weekday: "long",
        day: "numeric",
        month: "long",
        year: "numeric",
    ...
    <div className="space-y-8 max-w-[1200px] mx-auto pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Gestión de ingresos a planta y validación automática de HSE.
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="registro" className="space-y-6">
        <TabsList className="bg-slate-100">
          <TabsTrigger value="registro" className="font-semibold">
            <DoorOpen className="w-4 h-4 mr-2" /> Registro en Vivo
          </TabsTrigger>
          <TabsTrigger value="reportes" className="font-semibold">
            <FileCheck className="w-4 h-4 mr-2" /> Reportes de Tránsito
          </TabsTrigger>
        </TabsList>
            ...
            <Card className="border-slate-200 shadow-sm border-t-4 border-t-blue-600">
              <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4">
                <CardTitle className="text-lg flex items-center gap-2">
                  <Search className="w-5 h-5 text-blue-600" /> Identificación
                </CardTitle>
                <CardDescription>
                  Escanee o digite la cédula del trabajador.
                </CardDescription>
              </CardHeader>
              <CardContent className="pt-6">
                <form onSubmit={handleBuscar} className="space-y-4">
                  ...
                  <Button
                    type="submit"
                    className="w-full h-12 text-md font-bold bg-slate-900 text-white hover:bg-slate-800"
                  >
                    Verificar Estado
                  </Button>
                </form>
                ...
                {estadoAcceso && (
                  ...
                  >
                    ...
                    <ul
                      className={`text-sm space-y-1 mb-4 ${estadoAcceso.tipo === "NO_ENCONTRADO" ? "text-slate-600 font-medium" : "text-slate-600"}`}
                    >
                      {estadoAcceso.detalles.map((d, i) => (
                    ...
                    {estadoAcceso.tipo === "PERMITIDO" && (
                      <div className="space-y-4 pt-4 border-t border-green-200">
                        ...
                        >
                          <DoorOpen className="w-5 h-5 mr-2" /> REGISTRAR
                          ENTRADA
                        </Button>
                      </div>
                    )}
                    ...
                    {estadoAcceso.tipo === "SALIDA_LIBRE" && (
                      <div className="pt-4 border-t border-blue-200">
                        <Button
                          ...
                          className="w-full bg-[#0a1e36] hover:bg-[#163354] text-white font-bold h-12"
                        >
                    ...
                    {estadoAcceso.tipo === "NO_ENCONTRADO" && (
                      <div className="pt-4 border-t border-slate-300">
                        <Button
                          ...
                          variant="outline"
                          className="w-full text-slate-700 font-bold h-12 border-slate-300"
                        >
                    ...
                    {estadoAcceso.tipo === "BLOQUEADO" && (
                      <div className="pt-4 border-t border-red-200">
                        <Button
                          ...
                          variant="outline"
                          className="w-full text-red-700 font-bold h-12 border-red-200 bg-white hover:bg-red-50"
                        >
                          CANCELAR INGRESO
                        </Button>
                      </div>
              ...
              <CardContent className="p-4 flex flex-col flex-1">
                <div className="flex items-center gap-2 mb-4">
                  <div className="flex gap-2 flex-1">
                    ...
                    <Select
                      value={filtroTiempoVivo}
                      onValueChange={(val: any) => setFiltroTiempoVivo(val)}
                    >
                  ...
                  <div className="flex gap-1 bg-slate-100 p-1 rounded-md text-sm font-medium">
                    <button
                      onClick={() => setTransitosFiltroTipo("TODOS")}
                      className={`px-3 py-1 rounded-md transition-colors ${transitosFiltroTipo === "TODOS" ? "bg-[#0a1e36] text-white" : "text-slate-600 hover:bg-slate-200"}`}
                    >
                      Todos
                    </button>
                    <button
                      onClick={() => setTransitosFiltroTipo("ENTRADA")}
                      className={`px-3 py-1 rounded-md transition-colors ${transitosFiltroTipo === "ENTRADA" ? "bg-green-100 text-green-800" : "text-slate-600 hover:bg-slate-200"}`}
                    >
                      Ver Entradas
                    </button>
                    <button
                      onClick={() => setTransitosFiltroTipo("SALIDA")}
                      className={`px-3 py-1 rounded-md transition-colors ${transitosFiltroTipo === "SALIDA" ? "bg-red-100 text-red-800" : "text-slate-600 hover:bg-slate-200"}`}
                    >
                      Ver Salidas
                    </button>
                  </div>
                  ...
                  <Table>
                    <TableHeader className="bg-white sticky top-0 z-10 shadow-sm">
                      <TableRow className="border-b-2">
                        <TableHead className="w-[80px] font-bold text-slate-800 text-xs uppercase">
                          Hora
                        </TableHead>
                        <TableHead className="font-bold text-slate-800 text-xs uppercase">
                          Personal
                        </TableHead>
                        <TableHead className="w-[120px] text-center font-bold text-slate-800 text-xs uppercase">
                          Registro
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                        ...
                        Object.keys(registrosAgrupados).map((dateKey) => (
                          <React.Fragment key={dateKey}>
                            <TableRow className="bg-slate-50 hover:bg-slate-50">
                              <TableCell
                                colSpan={3}
                                className="font-bold text-slate-700 py-2 border-b"
                              >
                                {dateKey}
                              </TableCell>
                            </TableRow>
                              ...
                              >
                                ...
                                <TableCell className="py-3">
                                  <div className="flex items-center gap-2">
                                    <div className="font-black text-slate-900 group-hover:text-blue-700 transition-colors">
                                      {r.trabajadores?.nombre ||
                                        r.visitantes?.nombre}
                                    </div>
                                    {r.visitantes && (
                                      <Badge className="bg-purple-100 text-purple-800 hover:bg-purple-100 text-[9px] px-1.5 py-0 h-4">
                                        VISITANTE
                                      </Badge>
                                    )}
                                  </div>
                                  <div className="text-[11px] text-slate-500 font-medium">
                                    {r.trabajadores?.empresa ||
                                      r.visitantes?.empresa_origen ||
                                      "Visitante"}
                                  </div>
                                </TableCell>
                                <TableCell className="text-center align-middle py-3">
                                  <div className="flex justify-center">
                                    <Badge
                                      className={`w-20 justify-center h-6 rounded-full flex items-center ${r.tipo === "ENTRADA" ? "bg-green-100 text-green-700 hover:bg-green-100" : "bg-red-100 text-red-700 hover:bg-red-100"}`}
                                    >
                                      {r.tipo}
                                    </Badge>
                                  </div>
        ...
        <TabsContent value="reportes">
          <Card className="border-slate-200 shadow-sm">
            <CardHeader className="bg-slate-50 border-b border-slate-100">
              ...
              <CardDescription>
                Filtra y exporta las entradas y salidas de la planta.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-6">
              <div className="flex flex-col sm:flex-row gap-4 mb-6">
                <Select
                  value={filtroRango}
                  onValueChange={(val) => setFiltroRango(val || "")}
                >
                ...
                <Select
                  value={filtroEmpresa}
                  onValueChange={(val) => setFiltroEmpresa(val || "")}
                >
                ...
                <Select
                  ...
                  onValueChange={(val) => setFiltroProyecto(val || "")}
                >
                  <SelectTrigger className="w-full sm:w-[220px]">
                      ...
                      <TableRow key={r.id}>
                        ...
                        <TableCell>
                          <div className="font-bold text-slate-800 text-sm">
                            {r.trabajadores?.nombre || r.visitantes?.nombre}
                          </div>
                          <div className="text-xs text-slate-500">
                            C.C.{" "}
                            {r.trabajadores?.documento ||
                              r.visitantes?.documento}
                          </div>
                        </TableCell>
                        ...
                        <TableCell className="text-sm text-slate-600">
                          {r.proyectos?.nombre || "Planta General"}
                        </TableCell>
