...
const mockContratistasStats = [
  ...
  {
    id: 2,
    empresa: "Pinturas Marinas SAS",
    especialidad: "Sandblasting",
    asignados: 12,
    habilitacion: 85,
    docPendientes: 3,
  },
...
export default function ProyectosPage() {
  ...
  const [asignandoContratista, setAsignandoContratista] = useState<
    Record<string, string>
  >({}); // {actividadId: contratistaId}

  // CSV Upload State
  const fileInputActividadesRef = useRef<HTMLInputElement>(null);
  const [isCsvActividadesModalOpen, setIsCsvActividadesModalOpen] =
    useState(false);
  const csvActividadesTemplate =
    "data:text/csv;charset=utf-8,%EF%BB%BFNombre%0APintura de Casco%0ALimpieza de Tanques%0ASoldadura en Cubierta";
    ...
    if (pData) {
      const proyectosWithAforo = pData.map((p) => {
        const aforo = tData
          ? tData.filter(
              (t) =>
                t.proyecto_asignado === p.nombre && insideWorkers.has(t.id),
            ).length
          : 0;
        return { ...p, aforo };
      });
  ...
  const computeStatsForContratistas = async (contratistasList: any[]) => {
    ...
    const getGlobalStatus = (docs: any) => {
      ...
      Object.values(docs).forEach((doc: any) => {
        if (doc) {
          ...
          if (doc.estado_aprobacion !== "Aprobado" && doc.estado !== "Faltante")
            hasNoAprobado = true;
        }
      });
    ...
    return contratistasList.map((c: any) => {
      const cWorkers = (workersData || []).filter(
        (w) =>
      ...
      cWorkers.forEach((worker) => {
        const docsMap: any = {};
        const requiredKeys = requiredDocsByCargo[worker.cargo] || [
          "ss",
          "examen",
        ];
        ...
        docsData
          ?.filter((d) => d.trabajador_id === worker.id)
          .forEach((d) => {
        ...
        if (
          globalStatus === "HABILITADO" ||
          globalStatus === "ALERTA PREVENTIVA"
        )
          habilitados++;
      });

      const totalWorkers = cWorkers.length;
      const aptoPorcentaje =
        totalWorkers > 0 ? Math.round((habilitados / totalWorkers) * 100) : 0;
  ...
  const fetchActividades = async (proyectoId: string) => {
    ...
    if (data) {
      ...
      const stats = await computeStatsForContratistas(
        Array.from(uniqueContratistas.values()),
      );
  ...
  const handleAddActividad = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        `*, actividad_contratistas(contratista_id, contratistas(nombre, especialidad))`,
      );
  ...
  const handleCsvUploadActividades = (
  ...
  const handleAssignContratista = async (actividadId: string) => {
    ...
    if (!error) {
      ...
      alert(
        "Error al asignar: El contratista podría ya estar asignado a esta actividad.",
      );
    }
  };
  ...
  return (
    <div className="space-y-8 max-w-[1200px] mx-auto pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">
            Directorio de Proyectos y Barcos
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Gestión de personal asignado, actividades y estado documental HSE
            por proyecto.
          </p>
        </div>
        <Dialog
            ...
            <div className="px-6 py-4 bg-slate-900 text-white border-b border-slate-800 relative flex items-start justify-between">
              <div>
                ...
                <DialogDescription className="text-slate-400 mt-1">
                  Crea un nuevo proyecto o registra un barco para asignar
                  contratistas.
                </DialogDescription>
              </div>
              <Button
      ...
      <div className="flex items-center gap-4 bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
        ...
        <div className="flex gap-2 border-r border-slate-200 pr-4">
          <Button
            ...
            className={
              viewMode === "grid" ? "bg-slate-900 text-white" : "bg-white"
            }
          >
          ...
          <Button
            ...
            className={
              viewMode === "list" ? "bg-slate-900 text-white" : "bg-white"
            }
          >
          ...
          {proyectos.map((proyecto) => (
            ...
            >
              <div className="h-2 w-full bg-slate-100 flex shrink-0">
                <div
                  className={`h-full ${getSaludColor(proyecto.salud)} w-full`}
                ></div>
              </div>
              <CardHeader className="pb-3 flex-1">
                <div className="flex justify-between items-start">
                  <Badge
                    variant="outline"
                    className="bg-slate-50 text-slate-600 border-slate-200 font-semibold mb-2"
                  >
                    {proyecto.tipo}
                  </Badge>
                  <Badge
                ...
                <div className="grid grid-cols-2 gap-4 mt-2">
                  <div className="bg-slate-50 rounded p-3 border border-slate-100">
                    <div className="text-[10px] font-bold text-slate-500 mb-1 flex items-center gap-1">
                      <Users className="w-3 h-3" /> AFORO ACTUAL
                    </div>
                  ...
                  <div className="bg-slate-50 rounded p-3 border border-slate-100">
                    <div className="text-[10px] font-bold text-slate-500 mb-1 flex items-center gap-1">
                      <FileCheck className="w-3 h-3" /> SALUD HSE
                    </div>
                    <div className="text-sm font-semibold text-slate-900 flex items-center gap-1.5 mt-1">
                      <div
                        className={`w-2 h-2 rounded-full ${getSaludColor(proyecto.salud)}`}
                      ></div>
                      {proyecto.salud}
                    </div>
                  </div>
              ...
              <CardFooter className="bg-slate-50 border-t border-slate-100 px-6 py-3 shrink-0 mt-auto">
                <Button
                  variant="link"
                  onClick={() => openDetailsModal(proyecto)}
                  className="px-0 text-slate-600 hover:text-slate-900 font-semibold text-sm w-full justify-between"
                >
              ...
              {proyectos.map((proyecto) => (
                <TableRow key={proyecto.id} className="hover:bg-slate-50/50">
                  ...
                  <TableCell className="text-center">
                    <Badge
                      variant="outline"
                      className={`${proyecto.salud === "Optimo" ? "bg-green-50 text-green-700 border-green-200" : proyecto.salud === "Riesgo Medio" ? "bg-yellow-50 text-yellow-700 border-yellow-200" : "bg-red-50 text-red-700 border-red-200"}`}
                    >
                      {proyecto.salud}
                    </Badge>
                  </TableCell>
      ...
      <Dialog open={isDetailsModalOpen} onOpenChange={setIsDetailsModalOpen}>
        <DialogContent
          className="sm:max-w-[750px] p-0 overflow-hidden"
          showCloseButton={false}
        >
          {selectedProyecto && (
            <>
              <div className="px-6 py-4 bg-slate-900 text-white border-b border-slate-800 relative flex items-start justify-between">
                <div>
                  ...
                  <DialogDescription className="text-slate-400 mt-1">
                    Configuración de HSE y asignación de contratistas por
                    actividad.
                  </DialogDescription>
                </div>
                <Button
              ...
              <div className="p-6">
                <Tabs defaultValue="actividades">
                  ...
                  <TabsContent value="actividades" className="space-y-6">
                    <Dialog
                      open={isCsvActividadesModalOpen}
                      onOpenChange={setIsCsvActividadesModalOpen}
                    >
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle>Carga Masiva de Actividades</DialogTitle>
                          <DialogDescription>
                            Sube un archivo .csv para agregar múltiples
                            actividades al mismo tiempo. Sigue estos pasos:
                            <br />
                            1. Descarga la plantilla base.
                            <br />
                            2. Llena la columna "Nombre" con tus actividades.
                            <br />
                        ...
                        <div className="py-6 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-lg bg-slate-50">
                          ...
                          <Button
                            variant="secondary"
                            onClick={() =>
                              fileInputActividadesRef.current?.click()
                            }
                          >
                        ...
                        <DialogFooter>
                          <a
                            href={csvActividadesTemplate}
                            download="plantilla_actividades.csv"
                          >
                            <Button
                              variant="link"
                              size="sm"
                      ...
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="flex gap-2">
                          {selectedProyecto.cronograma_pdf && (
                            <a
                              href={selectedProyecto.cronograma_pdf}
                              target="_blank"
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => setIsCsvActividadesModalOpen(true)}
                            className="h-8"
                          >
                    ...
                    <div className="space-y-4 max-h-[40vh] overflow-y-auto pr-2">
                      {actividades.length === 0 ? (
                        <p className="text-center text-slate-500 py-8 italic text-sm">
                          No hay actividades creadas en este proyecto aún.
                        </p>
                      ) : (
                        actividades.map((act) => (
                          <Card
                          ...
                          >
                            <div className="bg-slate-50 px-4 py-3 border-b border-slate-100 flex justify-between items-center">
                              ...
                              <Badge
                                variant="outline"
                                className="text-[10px] tracking-wider font-bold text-slate-500"
                              >
                                {act.estado}
                              </Badge>
                            </div>
                            <div className="p-4 bg-white">
                              <Label className="text-xs text-slate-500 font-bold mb-2 block uppercase tracking-wider">
                              ...
                              {act.actividad_contratistas?.length > 0 ? (
                                ...
                                <p className="text-xs text-slate-400 italic mb-4">
                                  No hay contratistas asignados a esta
                                  actividad.
                                </p>
                              )}
                              ...
                              <div className="flex gap-2 items-end mt-2 pt-2 border-t border-slate-100">
                                <div className="flex-1">
                                  <Select
                                    ...
                                    onValueChange={(val) =>
                                      setAsignandoContratista({
                                        ...asignandoContratista,
                                        [act.id]: val,
                                      })
                                    }
                                  >
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    handleAssignContratista(act.id)
                                  }
                  ...
                  <TabsContent value="resumen" className="space-y-4">
                    <div className="flex gap-4 mb-4 bg-slate-50 p-4 rounded-lg border border-slate-200">
                      <div className="flex-1 text-center border-r border-slate-200">
                        ...
                        <p className="font-bold flex justify-center items-center gap-1.5">
                          <div
                            className={`w-2 h-2 rounded-full ${getSaludColor(selectedProyecto.salud)}`}
                          ></div>
                          {selectedProyecto.salud}
                        </p>
                      </div>
                      <div className="flex-1 text-center">
                        ...
                        <p className="font-bold text-slate-900">
                          {selectedProyecto.aforo} trabajadores
                        </p>
                      </div>
                    </div>

                    <h3 className="text-sm font-semibold text-slate-900 mt-4 mb-2">
                      Desempeño General Contratistas
                    </h3>
                        ...
                        <TableBody>
                          {projectStats.length > 0 ? (
                            projectStats.map((c) => (
                              <TableRow key={c.id}>
                                ...
                                <TableCell className="text-right text-xs">
                                  <span
                                    className={`font-bold ${c.habilitacion >= 95 ? "text-green-600" : c.habilitacion >= 80 ? "text-yellow-600" : "text-red-600"}`}
                                  >
                                    {c.habilitacion}%
                                  </span>
                                </TableCell>
                            ...
                            <TableRow>
                              <TableCell
                                colSpan={3}
                                className="text-center py-4 text-xs text-slate-500 italic"
                              >
                                No hay contratistas asignados a las actividades
                                de este proyecto.
                              </TableCell>
                            </TableRow>
                          )}
