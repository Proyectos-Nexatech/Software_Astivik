...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    if (workers.length > 0 && docs) {
      workers.forEach((w) => {
        ...
        reqDocs.forEach((req) => {
          ...
          if (
            !doc ||
            doc.estado_aprobacion !== "Aprobado" ||
            doc.estado === "Vencido" ||
            doc.estado === "Faltante"
          ) {
            isApto = false;
          ...
          if (
            doc &&
            (doc.estado_aprobacion === "Rechazado" || doc.estado === "Vencido")
          ) {
            alertas.push({
              trabajador: w.nombre,
              empresa: w.empresa,
              documento: req,
              estado: doc.estado,
              aprobacion: doc.estado_aprobacion,
            });
          }
        });
    ...
    const globalCompliance =
      workers.length > 0 ? (aptosTotales / totalWorkers) * 100 : 0;

    const contratistasInfo = Object.entries(statsPorEmpresa)
      .map(([empresa, data]) => ({
        empresa,
        total: data.total,
        aptos: data.aptos,
    ...
    let queryAccesos = supabase.from("registros_acceso").select("*");

    if (filtroProyecto !== "all") {
      const selectedProjectId = pData?.find(
        (p: any) => p.nombre === filtroProyecto,
      )?.id;
    ...
    if (accesos) {
      ...
      accesos.forEach((a) => {
        ...
        if (
          !existing ||
          new Date(a.fecha_hora) > new Date(existing.fecha_hora)
        ) {
          latestAccessMap.set(idKey, a);
        }
      });
      aforo = Array.from(latestAccessMap.values()).filter(
        (a) => a.tipo === "ENTRADA",
      ).length;
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Datos en tiempo real de seguridad y aforo en planta.
          </p>
        </div>
      </div>
      ...
      <Card className="bg-white shadow-sm border-slate-200">
        <CardContent className="p-4 flex flex-wrap md:flex-nowrap gap-4 items-end">
          ...
          <Button
            ...
            className="bg-[#0a1e36] hover:bg-[#163354] text-white font-semibold h-10 px-6"
          >
        ...
        <Card className="shadow-sm border-slate-200">
          <CardHeader className="pb-2">
            <CardTitle className="text-[11px] font-bold text-slate-500 tracking-wider flex justify-between items-center">
              ...
              <div
                className={`w-5 h-5 rounded-full border flex items-center justify-center ${stats.cumplimientoGlobal >= 90 ? "border-green-500" : "border-yellow-500"}`}
              >
                <div
                  className={`w-2.5 h-2.5 rounded-full ${stats.cumplimientoGlobal >= 90 ? "bg-green-500" : "bg-yellow-500"}`}
                ></div>
              </div>
            </CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="mt-5 h-2.5 w-full bg-slate-100 rounded-full overflow-hidden flex">
              <div
                className={`${stats.cumplimientoGlobal >= 90 ? "bg-green-500" : "bg-yellow-500"} rounded-full transition-all duration-1000`}
                style={{ width: `${stats.cumplimientoGlobal}%` }}
              ></div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs font-medium text-blue-200">
                documentos rechazados/vencidos
              </span>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="border border-slate-200 rounded-lg p-5 bg-white shadow-sm">
              <h4 className="text-[11px] font-bold tracking-wider text-slate-600 flex items-center gap-2 mb-5">
                <Users className="w-3.5 h-3.5" /> ESTADO DE HABILITACIÓN POR
                EMPRESA
              </h4>
                  ...
                  <div key={c.empresa}>
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span className="font-mono text-slate-600 font-medium text-xs">
                        ...
                        aptos ({c.pct.toFixed(0)}%)
                      </span>
                    </div>
                    <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                      <div
                        className={`${c.pct >= 90 ? "bg-green-500" : c.pct >= 60 ? "bg-yellow-500" : "bg-red-500"} h-full transition-all`}
                        style={{ width: `${c.pct}%` }}
                      ></div>
                    </div>
                  </div>
            ...
            <div className="border border-slate-200 rounded-lg p-5 bg-white shadow-sm">
              <h4 className="text-[11px] font-bold tracking-wider text-red-600 flex items-center gap-2 mb-5">
                <AlertCircle className="w-3.5 h-3.5" /> ALERTAS Y RECHAZOS
                RECIENTES
              </h4>
