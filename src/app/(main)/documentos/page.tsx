...
import {
  notificarHseDocumentoSubido,
  notificarContratistaDocumentoAprobado,
...
export default function DocumentosHSEPage() {
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("hse_docs").getPublicUrl(filePath);

      // Save to DB
      const currentDoc = selectedTrabajador.docs[docKey];
      const vigencia =
        currentDoc?.vigencia || new Date().toISOString().split("T")[0];
      ...
      if (userProfile?.rol === "lider_contratista") {
        await notificarHseDocumentoSubido(
          selectedTrabajador.contratista,
  ...
  const handleAprobarDocumento = async (docKey: string, aprobado: boolean) => {
    ...
    if (!error) {
      if (aprobado) {
        ...
        await notificarContratistaDocumentoAprobado(
          "proyectos@nexatech.com.co",
          selectedTrabajador.nombre,
          docKey,
        );
      }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
        ...
        <span className="text-slate-600 font-medium animate-pulse">
          Cargando base de datos y permisos...
        </span>
      </div>
    );
  }
  ...
  const esLiderHSE =
    userProfile?.rol === "lider_hse" ||
    userProfile?.rol?.toLowerCase() === "administrador";
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">
            Control Documental HSE
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Matriz de certificados y aprobaciones.
          </p>
        </div>
        ...
        <CardContent className="p-0 overflow-x-auto">
          <Table>
            <TableHeader className="bg-slate-50">
              <TableRow>
                <TableHead className="font-bold text-slate-700 min-w-[250px] sticky left-0 bg-slate-50 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                  Personal / Empresa
                </TableHead>
                <TableHead className="font-bold text-slate-700 text-center min-w-[120px]">
                  Estado Global
                </TableHead>
                ...
                <TableHead className="text-center text-slate-600 min-w-[150px]">
                  Cert. Soldadura
                </TableHead>
                <TableHead className="text-center font-bold text-slate-700 sticky right-0 bg-slate-50 shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                  Acciones
                </TableHead>
              </TableRow>
            </TableHeader>
                  ...
                  >
                    <TableCell className="sticky left-0 bg-white shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                      ...
                      <div className="text-xs text-slate-500 font-medium">
                        {t.cargo} • {t.contratista}
                      </div>
                    </TableCell>
                    <TableCell className="text-center">
                      <Badge
                        variant="outline"
                        className={`${globalColor} whitespace-nowrap`}
                      >
                        {globalStatus === "HABILITADO" && (
                      ...
                      return (
                        <TableCell key={docKey} className="text-center">
                          <div className="flex flex-col items-center gap-1">
                            <Badge
                              variant="outline"
                              className={`${getStatusColor(doc.estado)} text-[10px]`}
                            >
                              {doc.estado}
                            </Badge>
                            {doc.estado !== "Faltante" && (
                              <span
                                className={`text-[10px] font-bold ${isAprobado ? "text-green-600" : isRechazado ? "text-red-600" : "text-blue-500"}`}
                              >
                                {isAprobado
                                  ? "✓ Aprobado"
                                  : isRechazado
                    ...
                    <TableCell className="text-center sticky right-0 bg-white shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                      <Button
                        ...
                        className="font-semibold text-blue-700 bg-blue-50 hover:bg-blue-100"
                      >
                        Gestionar
                      </Button>
                    </TableCell>
                  ...
                  >
                    ...
                    {userProfile?.rol === "lider_contratista" && (
                      <span className="block mt-2 text-xs text-blue-500">
                        (Tu empresa asignada es:{" "}
                        <strong>
                          {userProfile?.contratistas?.nombre || "Ninguna"}
                        </strong>
                        . Verifica que el personal en la base de datos tenga
                        exactamente este mismo nombre en el campo "empresa").
                      </span>
                    )}
                  </TableCell>
                </TableRow>
        ...
        <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl flex items-center gap-2">
              ...
              {esLiderHSE
                ? "Revisión HSE (Aprobaciones)"
                : "Gestión de Expediente"}
            </DialogTitle>
            <DialogDescription>
              {selectedTrabajador?.nombre} • {selectedTrabajador?.cargo} •{" "}
              {selectedTrabajador?.contratista}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              {Object.keys(selectedTrabajador.docs).map((docKey) => {
                ...
                return (
                  <div
                    key={docKey}
                    className={`flex flex-col lg:flex-row lg:items-center flex-wrap justify-between gap-4 p-4 rounded-lg border ${isAprobado ? "border-green-200 bg-green-50/30" : isRechazado ? "border-red-200 bg-red-50/30" : "border-slate-200 bg-white"}`}
                  >
                    <div className="flex-1 space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        {doc.estado !== "Faltante" && (
                          <Badge
                            variant="outline"
                            className={`text-[10px] ${isAprobado ? "bg-green-100 text-green-800" : isRechazado ? "bg-red-100 text-red-800" : "bg-blue-100 text-blue-800"}`}
                          >
                            {isAprobado
                              ? "Aprobado por HSE"
                              : isRechazado
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-2 bg-slate-50 p-1.5 rounded-md border border-slate-200">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateDocDate(docKey, e.target.value)
                          }
                      ...
                      {esLiderHSE && doc.estado !== "Faltante" && (
                        <div className="flex items-center gap-1 border-l border-slate-200 pl-2 ml-1">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              handleAprobarDocumento(docKey, false)
                            }
                          >
